const help = require("./helpersMidds/helpersMidd.js");
...
const validateJug = (req, res, next) => {
  ...
  if (!xCapacity) {
    return res.status(400).json({ error: "x_capacity: Invalid parameter" });
  }
  ...
  if (!yCapacity) {
    return res.status(400).json({ error: "y_capacity: Invalid parameter" });
  }
